 ...
 */
export async function getTopPages({ startDate, endDate, rowLimit = 25000 }: { startDate: string; endDate: string; rowLimit?: number }): Promise<TopPageResult[]> {
	try {
 ...
 */
export async function getTopQueriesForPage({ startDate, endDate, pageUrl, rowLimit = 1000 }: { startDate: string; endDate: string; pageUrl: string; rowLimit?: number }): Promise<TopQueryResult[]> {
	try {
		const encodedSiteUrl = encodeURIComponent(config.siteUrl);
		const endpoint = `sites/${encodedSiteUrl}/searchAnalytics/query`;
 ...
 */
export async function getDailySearchData({ startDate, endDate }: { startDate: string; endDate: string }): Promise<
	Array<{
		date: string;
		clicks: number;
		impressions: number;
