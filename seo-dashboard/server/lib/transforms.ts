...
export function computeUpdatePriority(gscCurrent: any[], ga4Current: any[]) {
  ...
  for (const pagePath of all) {
    ...
    if (gsc) {
      if (gsc.impressions > 1000 && gsc.ctr < 0.02) { priority += 30; reasons.push('High impressions, low CTR'); }
      if (gsc.impressions > 500 && gsc.position > 10) { priority += 25; reasons.push('High impressions, poor position'); }
      if (gsc.position <= 5 && gsc.ctr < 0.03) { priority += 20; reasons.push('Good position, low CTR'); }
    }
    if (ga4) {
      if (ga4.sessions > 100 && ga4.averageSessionDuration < 30) { priority += 25; reasons.push('High traffic, low engagement'); }
      if (ga4.sessions > 50) { priority += 15; reasons.push('High traffic page'); }
    }
    if (priority > 0) {
