...
export default defineEventHandler(async (event) => {
	...
	try {
		...
		const [gscCurrent, ga4Current, gscPrevious, ga4Previous] = await Promise.all([getTopPages(startDate, endDate, 1000), getPagesAggregate(startDate, endDate), getTopPages(previousRange.startDate, previousRange.endDate, 1000), getPagesAggregate(previousRange.startDate, previousRange.endDate)]);
